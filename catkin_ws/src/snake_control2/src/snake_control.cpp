...
int main(int argc, char **argv) {
    ...
    double sample_time = 1/timesteps_per_second;
    double prev_time = 0;
    double theta_0 = 0;
    IK_results1 = IK(x, y, theta, l1, l2, l3);
    int count = 0;
    int count1 = 0;

    PolynomialResult result1 = calculateFifthOrderPolynomial(t_f, timesteps_per_second, theta_0, IK_results1[0], 0, 0, 0, 0);
    PolynomialResult result2 = calculateFifthOrderPolynomial(t_f, timesteps_per_second, theta_0, IK_results1[2], 0, 0, 0, 0);
    PolynomialResult result3 = calculateFifthOrderPolynomial(t_f, timesteps_per_second, theta_0, IK_results1[4], 0, 0, 0, 0);


    IK_results2 = IK(10, 20, 0.8, l1, l2, l3);

    PolynomialResult result1_1 = calculateFifthOrderPolynomial(t_f, timesteps_per_second, result1.theta[result1.theta.size()-1], IK_results2[0], 0, 0, 0, 0);
    PolynomialResult result2_2 = calculateFifthOrderPolynomial(t_f, timesteps_per_second, result2.theta[result1.theta.size()-1], IK_results2[2], 0, 0, 0, 0);
    PolynomialResult result3_3 = calculateFifthOrderPolynomial(t_f, timesteps_per_second, result3.theta[result1.theta.size()-1], IK_results2[4], 0, 0, 0, 0);

    std::vector<double> result1_all = result1.theta;
    result1_all.insert(result1_all.end(), result1_1.theta.begin(), result1_1.theta.end());
    std::vector<double> result2_all = result2.theta;
    result2_all.insert(result2_all.end(), result2_2.theta.begin(), result2_2.theta.end());
    std::vector<double> result3_all = result3.theta;
    result3_all.insert(result3_all.end(), result3_3.theta.begin(), result3_3.theta.end());
    ...
    while (ros::ok()) {
        ...
        if(current_time - prev_time >= sample_time && time >= 5 && count < (result1.time.size() + result1_1.time.size() )){
            angles_to_publish[0].data = result1_all[count];
            angles_to_publish[1].data = 0;
            angles_to_publish[2].data = result2_all[count];
            angles_to_publish[3].data = 0;
            angles_to_publish[4].data = result3_all[count++];
